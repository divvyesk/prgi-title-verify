...
configured (agents/clients.py) — FixtureVerifyClient today, HttpVerifyClient
from Prompt 6 onward, once it's wired to the real live `/v1/verify`-backed
endpoint. This file never simplifies that check itself: a suggested title
that only passed a cheaper "quick check" would make the whole feature's
guarantee ("every candidate is verified by the same engine that verifies an
applicant's own title") worthless.
"""

from __future__ import annotations

from agents.clients import FixtureVerifyClient, VerifyClient
from agents.state import StudioState

_default_client: VerifyClient = FixtureVerifyClient()
